...
ReAct、Plan 单任务与 SubAgent 共用 `AgentBudget`。系统属性 > 默认值：`paicli.react.token.budget`(Integer.MAX_VALUE) / `paicli.react.stagnation.window`(3) / `paicli.react.hard.max.iterations`(不限制)。

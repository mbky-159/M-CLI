...
本阶段目标：把 M-CLI 从“使用 JLine 的 CLI”升级为“以 JLine 4 为核心 UI runtime 的 Agent 终端”，交互体验对齐 Claude Code / Qoder CLI 一类产品。
...
- 已接入 `PaiCliHistory`，基于 JLine `DefaultHistory` 做 M-CLI 过滤策略。
...
- 暂不使用 `jline-shell` 替换 M-CLI 现有命令解析。
...
- HITL 审批期间不吞输入。
- `java -jar target/m-cli-1.0-SNAPSHOT.jar` 真机验证。

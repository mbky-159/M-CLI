...
状态：单用户预发布已运行；多用户 API、租户沙箱调度与公网入口尚未交付。
...
1. **基础准备（已完成）**：沙箱镜像、离线 Compose 配置、可选 MySQL/Redis、静态策略测试、Linux gVisor 运行探针，以及带健康检查和回滚的单用户预发布流水线。实验探针通过不代表已交付租户沙箱服务。
...
已购买并初始化新加坡轻量应用服务器（Ubuntu 24.04、4 vCPU、8 GiB、70 GiB ESSD）。M-CLI 单用户预发布由 systemd 托管，只监听 `127.0.0.1:8080`，`/healthz` 验收通过；Docker 与 gVisor `runsc` 已安装，沙箱镜像已构建，动态探针确认非 root、只读根目录、loopback-only 网络、受限 tmpfs 和无模型 Key 注入。GitHub Actions 工作流已准备，仓库 Secrets 与部署开关仍待配置。没有创建公共业务入口或开放 Runtime API 端口。准备文件的使用方式见 [部署实验说明](../deploy/README.md)。

...
1. **基础准备（本次）**：沙箱镜像、离线 Compose 配置、可选 MySQL/Redis、静态策略测试和 Linux 运行探针。尚未运行验证的部分不能标记为已交付沙箱服务。
...
本地 Docker Compose 可以渲染；本机 Docker daemon 未启动，尚未构建沙箱镜像或执行 gVisor 探针。没有购买服务器、创建公共入口或开放端口。准备文件的使用方式见 [部署实验说明](../deploy/README.md)。
